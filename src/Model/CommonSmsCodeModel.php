...
class CommonSmsCodeModel extends Model
{
    ...
    static function addInfo($mobile, $type, $code, $number, $expired_time = 0, $create_time = 0) {
    	$postData = [
    		...
    		'code'=>$code,
    		'number'=>$number,
    		'expired_time'=>$create_time ? $create_time : hst_time() + 300,
    		'create_time'=> $create_time ? $create_time : hst_time()
    	];
	...
	static function _buildCode() {
		$length = hst_config('sms', 'codelength');
		$length = $length ? $length : 6;
		$code = hst_random($length, true);
		return $code;
	}
}
