...
class HstcmsDb
{
    ...
    public function table($table, Closure $callback)
    {
    	if(!$this->hasTable($table)) {
    		return hst_message('is has table');
    	}
    ...
    public function renameTable($from, $to)
    {
    	if(!$this->hasTable($from)) {
    		return hst_message('is no has table');
    	}
    	if($this->hasTable($to)) {
    		return hst_message('is has table '.$to);
    	}
    ...
    public function addColumn($table, $column, $columnAttribute = []) 
    {
    	...
    	if($this->hasColumn($table, $column)) {
    		return hst_message('is has column');
    	}
    ...
    public function renameColumn($table, $from, $to) 
    {
    	...
    	if(!$this->hasColumn($table, $from)) {
    		return hst_message('is no has column');
    	}
    	if($this->hasColumn($table, $to)) {
    		return hst_message('is has column '.$to);
    	}
