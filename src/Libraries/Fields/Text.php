...
class Text extends FieldAbs {
    ...
    {
		parent::__construct();
		$this->name = hst_lang('hstcms::fields.text'); // 字段名称
	...
	public function option($option) 
	{
		...
		return '
	        ...
	        <div class="hstui-form-group hstui-form-group-sm">
	          <label class="hstui-u-sm-2 hstui-form-label">'.hst_lang('hstcms::manage.fields.ispass').'</label>
	          <div class="hstui-u-sm-10 hstui-form-input">
                    <input type="radio" value="0" name="setting[option][ispass]" '.($option['ispass'] == 0 ? 'checked' : '').' >&nbsp;<label>'.hst_lang('hstcms::public.no').'</label>&nbsp;&nbsp;
                    <input type="radio" value="1" name="setting[option][ispass]" '.($option['ispass'] == 1 ? 'checked' : '').' >&nbsp;<label>'.hst_lang('hstcms::public.yes').'</label>&nbsp;&nbsp;
	            <div class="hstui-form-input-tips" data-tips="'.hst_lang('hstcms::manage.fields.ispass.tips').'">'.hst_lang('hstcms::manage.fields.ispass.tips').'</div>
	          </div>
	        </div>
