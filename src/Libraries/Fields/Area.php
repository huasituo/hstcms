...
class Area extends FieldAbs 
{
    ...
    {
		parent::__construct();
		$this->name = hst_lang('hstcms::fields.area').hst_lang('hstcms::fields.area.mode.003'); // 字段名称
		$this->fieldtype = [
			'INT' => '11'
	...
	public function option($option) 
	{
		...
		$areaid1 = '';
		$areaList = CommonAreaModel::getSubByAreaid(0, false);
		foreach ($areaList as $key => $value) {
			$areaid1 .= '<option value="'.$value['areaid'].'" '.hst_isSelected($value['areaid'] == $option['areaid1']).'>'.$value['name'].'</option>';
		}
		return '
			<div class="hstui-form-group hstui-form-group-sm">
	          ...
	          <div class="hstui-u-sm-10 hstui-form-input">
	            <select name="setting[option][areaid1]" class="hstui-input hstui-select J_areaAjax" data-toid="hstcms_areaid2" id="hstcms_areaid1" data-area="1" data-tips="请选择省份">
					<option value="">请选择省份</option>
					'.$areaid1.'
	            </select>
	            <select name="setting[option][areaid2]" class="hstui-input hstui-select J_areaAjax" data-toid="hstcms_areaid3" id="hstcms_areaid2" data-area="2" data-tips="请选择城市">
					<option value="">请选择城市</option>
	            </select>
	            <select name="setting[option][areaid3]" class="hstui-input hstui-select" data-area="3" id="hstcms_areaid3" data-tips="请选择县(区)">
	...
	public function createSql($data)
	{
        ...
        CommonFieldsModel::createSql($postData);
	...
	public function deleteSql($data)
	{
        ...
        CommonFieldsModel::deleteSql($datas);
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) 
	{
		...
		if (!$value && $areaid3) {
			$vInfo = CommonAreaModel::getInfoByAreaid($areaid3, false);
		...
		$areaList = CommonAreaModel::getSubByAreaid(0, false);
		foreach ($areaList as $key => $value) {
			$option1 .= '<option value="'.$value['areaid'].'" '.hst_isSelected($vInfo && $value['areaid'] == $vInfo['joinids'][0]).'>'.$value['name'].'</option>';
		}
		$str.= '<select '.$disabled.' name="'.$name.'1" class="hstui-input hstui-select J_areaAjax_'.$name.'" data-toid="hstcms_'.$name.'2" id="hstcms_'.$name.'1" data-area="1" data-tips="请选择省份">
					<option value="">请选择省份</option>
					'.$option1.'
	            </select>
	            <select '.$disabled.'  name="'.$name.'2" class="hstui-input hstui-select J_areaAjax_'.$name.'" data-toid="hstcms_'.$name.'3" id="hstcms_'.$name.'2" data-area="2" data-tips="请选择城市">
					<option value="">请选择城市</option>
	            </select>
	            <select '.$disabled.' name="'.$name.'3" class="hstui-input hstui-select" id="hstcms_'.$name.'3"  data-area="3" data-tips="请选择县(区)">
					<option value="">请选择县(区)</option>
	            </select>';
        $str .='
            <script>
            ...
            var '.$name.'3 = '.(int)$vInfo['joinids'][2].';
                Hstui.use(\'jquery\',function(){
                	...
                	if('.$name.'1) {
                		setArea_'.$name.'('.$name.'1, \'hstcms_'.$name.'2\', 1, '.$name.'2);
                	}
                	if('.$name.'2) {
                		setArea_'.$name.'('.$name.'2, \'hstcms_'.$name.'3\', 2, '.$name.'3);
                	}
					$(".J_areaAjax_'.$name.'").on(\'change\',function(){
						var _this=$(this),val=_this.val(),toid=_this.data(\'toid\'),area=_this.data(\'area\');
						setArea_'.$name.'(val, toid, area);
						if(area == 1) {
                			setArea_'.$name.'(0, \'hstcms_'.$name.'3\');
						}
					});
                });
            </script>';
	...
	public function output_data($data, $field = []) 
	{
		...
		$data[$field['fieldname']] = CommonAreaModel::getInfoByAreaid($value, false);
