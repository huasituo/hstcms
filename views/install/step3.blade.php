...
<form class="conts-form J_ajaxForm" action="{!! url('install/checkDatabase') !!}" method="post" >
            ...
            <div class="main" style="overflow:hidden; margin-bottom: 10px;">
                <div class="hstui-frame" style="width: 100% !important; ">
                    <div class="hstui-frame-title">
                        {{ hst_lang('hstcms::install.title1') }}
                    </div>
                    <div class="hstui-frame-content">
                        <div class="hstui-form hstui-form-horizontal" action="">
                            <div class="hstui-form-group @if($errors->first('site_url')) hstui-form-error @endif"  id="J_form_error_site_url">
                                <label for="exampleInputName2" class="hstui-u-sm-3 hstui-form-label">{{ hst_lang('hstcms::install.username') }}</label>
                                <input type="text" name="site_url" value="@if(old('site_url')){!! old('site_url') !!}@else{!! $preData['site_url'] !!}@endif">
                                <label class="Validform_checktip point-out">{{ hst_lang('hstcms::install.username') }}</label>
                            </div>
                            <div class="hstui-form-group @if($errors->first('db_host')) hstui-form-error @endif" id="J_form_error_db_host">
                                <label for="exampleInputName3" class="hstui-u-sm-3 hstui-form-label">{{ hst_lang('hstcms::install.db.host') }}</label>
                                <input type="text" name="db_host" value="@if(old('db_host')){!! old('db_host') !!}@else{!! $preData['db_host'] !!}@endif">
                                <label class="Validform_checktip point-out">@if($errors->first('db_host')){!! $errors->first('db_host') !!}@else {{ hst_lang('hstcms::install.db.host.tips') }} @endif</label>
                            </div>
                            <div class="hstui-form-group @if($errors->first('db_database')) hstui-form-error @endif" id="J_form_error_db_name">
                                <label for="exampleInputName4" class="hstui-u-sm-3 hstui-form-label">{{ hst_lang('hstcms::install.db.database') }}</label>
                                <input type="text" name="db_database" value="@if(old('db_database')){!! old('db_database') !!}@else{!! $preData['db_database'] !!}@endif">
                                <label class="Validform_checktip point-out">{{ hst_lang('hstcms::install.db.database.tips') }}</label>
                            </div>
                            <div class="hstui-form-group @if($errors->first('db_username')) hstui-form-error @endif" id="J_form_error_db_account">
                                <label for="exampleInputName5" class="hstui-u-sm-3 hstui-form-label">{{ hst_lang('hstcms::install.db.username') }}</label>
                                <input type="text" name="db_username" value="@if(old('db_username')){!! old('db_username') !!}@else{!! $preData['db_username'] !!}@endif">
                                <label class="Validform_checktip point-out">@if($errors->first('db_username')){!! $errors->first('db_username') !!}@else {{ hst_lang('hstcms::install.db.username.tips') }} @endif</label>
                            </div>
                            <div class="hstui-form-group @if($errors->first('db_password')) hstui-form-error @endif" id="J_form_error_db_password">
                                <label for="exampleInputName6" class="hstui-u-sm-3 hstui-form-label">{{ hst_lang('hstcms::install.db.password') }}</label>
                                ...
                                <label class="Validform_checktip point-out">@if($errors->first('db_password')){!! $errors->first('db_password') !!}@else {{ hst_lang('hstcms::install.db.password.tips') }} @endif</label>
                            </div>
                        </div>
                        <div class="hstui-frame-title" style="background: #fff; margin-bottom: 10px">
                        {{ hst_lang('hstcms::install.title2') }}
                        </div>
                        <div class="hstui-form hstui-form-horizontal" action="">
                            <div class="hstui-form-group @if($errors->first('username')) hstui-form-error @endif">
                                <label for="exampleInputName7" class="hstui-u-sm-3 hstui-form-label">{{ hst_lang('hstcms::install.username') }}</label>
                                <input type="text" name="username" value="@if(old('username')){!! old('username') !!}@else{!! $preData['username'] !!}@endif">
                                <label class="Validform_checktip point-out">{{ hst_lang('hstcms::install.username.tips') }}</label>
                            </div>
                            <div class="hstui-form-group @if($errors->first('password')) hstui-form-error @endif">
                                <label for="exampleInputName8" class="hstui-u-sm-3 hstui-form-label">{{ hst_lang('hstcms::install.password') }}</label>
                                <input type="password" name="password"  value="{!! old('password') !!}">
                                <label  class="Validform_checktip point-out">{{ hst_lang('hstcms::install.password.tips') }}</label>
                            </div>
                            <div class="hstui-form-group @if($errors->first('confirm_password')) hstui-form-error @endif">
                                <label for="exampleInputName9" class="hstui-u-sm-3 hstui-form-label">{{ hst_lang('hstcms::install.confirm.password') }}</label>
                                <input type="password" name="confirm_password" value="{!! old('confirm_password') !!}">
                                <label  class="Validform_checktip point-out">{{ hst_lang('hstcms::install.confirm.password.tips') }}</label>
                            </div>
                            <!-- <div class="radio">
    ...
    <footer>
        <div class="footer">
            ...
            <button class="btn-orange shadow_con J_ajax_submit_btn" >{{ hst_lang('hstcms::install.submit') }}</button>
        </div>
    </footer>
</form>
<script>
