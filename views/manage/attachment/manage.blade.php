...
{!! $navs !!}
    <div class="manage-search">
        <form action="{{ route('manageAttachManage') }}" method="get">
            ...
            <input class="hstui-input hstui-length-2" id="search-aid" name="aid" value="{!! hst_value('aid', $args) !!}" placeholder="AID" />
            <input class="hstui-input hstui-length-2" id="search-uid" name="uid" value="{!! hst_value('uid', $args) !!}" placeholder="UID" />
            <input class="hstui-input hstui-length-3" id="search-name" name="name" value="{!! hst_value('name', $args) !!}" placeholder="{!! hst_lang('hstcms::public.name') !!}" />
            <input class="hstui-input hstui-length-2" id="search-app" name="app" value="{!! hst_value('app', $args) !!}" placeholder="APP" />
            <input class="hstui-input hstui-length-2" id="search-appid" name="appid" value="{!! hst_value('appid', $args) !!}" placeholder="APPID" />
...
<div class="table-main">
    ...
    <table class="hstui-table hstui-table-bordered hstui-table-radius hstui-table-striped hstui-table-hover hstui-table-compact hstui-text-nowrap">
       <thead class="hstui-table-head">
            <tr>
                <th width="80" >{{ hst_lang('AID') }}</th>
                <th width="80" >{{ hst_lang('UID') }}</th>
                <th width="80" >{{ hst_lang('APP') }}</th>
                <th width="80" >{{ hst_lang('APPID') }}</th>
                <th width="20%" >{{ hst_lang('hstcms::public.name') }}</th>
                <th >{{ hst_lang('hstcms::public.type') }}</th>
                <th >{{ hst_lang('hstcms::public.size') }}</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{!! $v['name'] !!}</td>
                <td>{!! $v['type'] !!}</td>
                <td>{!! hst_byte_format($v['size']) !!}</td>
                <td>{!! hst_time2str($v['created_time']) !!}</td>
                <td>
                   <a class="J_dialog" title="{{ hst_lang('hstcms::public.view') }}" href="{{ route('manageAttachView', ['aid'=>$v['aid']]) }}">{{ hst_lang('hstcms::public.view') }}</a>
                </td>
            </tr>
            @endforeach
            @else
    ...
    @else
        <div class="hstui-attach-list">
            ...
            @if(count($list))
            @foreach($list as $v)
                <li class="J_tooltips" data-tooltips-content="{{$v['name']}}</br>{!! hst_byte_format($v['size']) !!}">
                    @if(in_array($v['type'], ['jpeg', 'jpg', 'png', 'gif']))
                    <a href="{{ $v['url'] }}" target="_b" class="view"><i class="hstui-icon hstui-icon-eye"></i></a>
                    @endif
                   <a class="J_dialog" title="{{ hst_lang('hstcms::public.view') }}" href="{{ route('manageAttachView', ['aid'=>$v['aid']]) }}"> 
                    @if(in_array($v['type'], ['jpeg', 'jpg', 'png', 'gif']))
                    <img src="{{ $v['url'] }}">
                    @endif
                    @if($v['type'] == 'zip')
